...
function createTextTexture(text, color, bgColor) {
    ...
    ctx.fillStyle = color; ctx.font = "bold 40px Consolas";
...
function createPenguinTexture() {
    ...
    ctx.beginPath(); ctx.ellipse(centerX, centerY, 52, 62, 0, 0, Math.PI * 2); ctx.fill();
    // Corps Noir
    ctx.fillStyle = "#111111";
    ctx.beginPath(); ctx.ellipse(centerX, centerY, 48, 58, 0, 0, Math.PI * 2); ctx.fill();
    // Ventre Blanc
    ctx.fillStyle = "#f0f0f0"; 
    ctx.beginPath();
    ctx.moveTo(centerX, centerY - 20);
    ctx.bezierCurveTo(centerX + 40, centerY - 10, centerX + 30, centerY + 50, centerX, centerY + 55);
    ctx.bezierCurveTo(centerX - 30, centerY + 50, centerX - 40, centerY - 10, centerX, centerY - 20);
    ctx.fill();
    // Yeux
    ctx.fillStyle = "#ffffff";
    ctx.beginPath(); ctx.arc(centerX - 15, centerY - 25, 10, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(centerX + 15, centerY - 25, 10, 0, Math.PI*2); ctx.fill();
    // Pupilles
    ctx.fillStyle = "#000000";
    ctx.beginPath(); ctx.arc(centerX - 15, centerY - 25, 5, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(centerX + 15, centerY - 25, 5, 0, Math.PI*2); ctx.fill();
    // Bec
    ctx.fillStyle = "#ff9900";
    ctx.beginPath(); ctx.moveTo(centerX - 10, centerY - 5); ctx.lineTo(centerX + 10, centerY - 5); ctx.lineTo(centerX, centerY + 15); ctx.fill();
    // Pieds
    ctx.beginPath(); ctx.ellipse(centerX - 25, centerY + 55, 15, 8, Math.PI / 4, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(centerX + 25, centerY + 55, 15, 8, -Math.PI / 4, 0, Math.PI * 2); ctx.fill();
...
function init() {
    ...
    bloomPass.threshold = 0; bloomPass.strength = 1.6; bloomPass.radius = 0.5;
...
function createWorld() {
    // Noyau XXL
    const geoCore = new THREE.IcosahedronGeometry(12, 2);
    const matCore = new THREE.MeshBasicMaterial({ color: 0x00ff00, wireframe: true });
    core = new THREE.Mesh(geoCore, matCore);
    const inner = new THREE.Mesh(new THREE.IcosahedronGeometry(9, 1), new THREE.MeshBasicMaterial({ color: 0xffffff }));
    core.add(inner);
    scene.add(core);
    ...
    for(let i=0; i<40; i++) {
        ...
        scene.add(mesh);
        windowsGroup.push(mesh);
    }
}

function createSpectrum() {
    spectrumGroup = new THREE.Group();
    const count = 64; const radius = 22;
    const barGeo = new THREE.BoxGeometry(0.5, 0.5, 0.5);
...
function launchPenguin() {
    const material = new THREE.SpriteMaterial({ map: penguinTex, color: 0xeeeeee });
    ...
    const angle = Math.random() * Math.PI * 2;
    // Vitesse ralentie
    const speedXY = 0.1 + Math.random() * 0.4; 
    const speedZ = 0.2 + Math.random() * 1.2;

    const velocity = new THREE.Vector3(
        Math.cos(angle) * speedXY,
        Math.sin(angle) * speedXY,
        speedZ
    );

    penguinsArray.push({ mesh: sprite, velocity: velocity });
    scene.add(sprite);
}
...
function animate() {
    ...
    if(analyser) {
        ...
        let bSum = 0; for(let i=0; i<10; i++) bSum += dataArray[i];
        bass = bSum / 10;
        let total = 0; for(let i=0; i<dataArray.length; i++) total += dataArray[i];
        avgVolume = total / dataArray.length;

        spectrumGroup.children.forEach((bar, i) => {
            const val = dataArray[i];
            const scale = 1 + (val / 10); 
            bar.scale.set(scale, 1, 1);
            const hue = 120 - (val / 255) * 120;
            bar.material.color.setHSL(hue / 360, 1, 0.5);
        });
    }

    // Gestion Couleur Noyau
    const r = Math.min(1, avgVolume / 150);
    const g = Math.max(0, 1 - (avgVolume / 200));
    
    core.children[0].material.color.setRGB(r, g, 0);
    core.material.color.setRGB(r, g, 0);
    ...
    // Condition de lancement (Surchauffe)
    if (avgVolume > 130 && canLaunch) {
        launchPenguin();
        canLaunch = false;
        setTimeout(() => { canLaunch = true; }, 150); 
    }

    // Mise à jour Pingouins
    for (let i = penguinsArray.length - 1; i >= 0; i--) {
        const p = penguinsArray[i];
        p.mesh.position.add(p.velocity);
        p.mesh.material.rotation += 0.01; // Rotation douce
        if (p.mesh.position.z > 60 || p.mesh.position.length() > 250) {
            scene.remove(p.mesh); penguinsArray.splice(i, 1);
        }
    }
    document.getElementById('penguin-count').innerText = penguinsArray.length;

    // Reste de l'animation
    const pulse = Math.pow(bass / 200, 3); 
    const scaleCore = 1 + pulse;
    core.scale.set(scaleCore, scaleCore, scaleCore);
    core.rotation.z += 0.01; core.rotation.y += 0.02;

    const targetFov = baseFov + (bass / 255) * 10;
    ...
    gridTop.position.z += 0.5 + (avgVolume/100);
    gridBottom.position.z += 0.5 + (avgVolume/100);
