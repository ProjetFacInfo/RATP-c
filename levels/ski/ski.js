// Importation de THREE depuis le fichier principal (l'instruction 'import' est gérée dans index.html)
...
belly.position.set(0, 2.3, -1.05);
tuxModel.add(belly);

// 3. Tête
const headGroup = new THREE.Group();
headGroup.position.set(0, 3.9, 0);
tuxModel.add(headGroup);

const head = new THREE.Mesh(new THREE.SphereGeometry(1.3, 32, 32), blackMat);
head.castShadow = true;
headGroup.add(head);

const face = new THREE.Mesh(new THREE.SphereGeometry(1.1, 32, 32), whiteMat);
face.scale.set(1, 1, 0.5);
face.position.set(0, 0, -0.9);
face.rotation.x = 0.1;
headGroup.add(face);
...
flipperL.position.set(-2.1, 2.8, 0); flipperL.rotation.set(0, 0, Math.PI/4); flipperL.scale.z=0.2; flipperL.castShadow=true;
tuxModel.add(flipperL);
const flipperR = flipperL.clone();
flipperR.position.set(2.1, 2.8, 0); flipperR.rotation.set(0, 0, -Math.PI/4);
tuxModel.add(flipperR);

// 5. Pieds
const feetGroup = new THREE.Group();
tuxModel.add(feetGroup);
const footGeo = new THREE.BoxGeometry(1.3, 0.3, 2.2);
const footL = new THREE.Mesh(footGeo, yellowMat);
footL.position.set(-1.1, 0.15, 0); footL.rotation.y=-0.1; footL.castShadow=true;
feetGroup.add(footL);
const footR = footL.clone();
footR.position.set(1.1, 0.15, 0); footR.rotation.y=0.1;
feetGroup.add(footR);
...
function animate() {
    ...
    if (gameState === 'charging') {
        ...
        // ANIMATION DE PLONGEON (Glisse)
        tuxModel.rotation.x = THREE.MathUtils.lerp(tuxModel.rotation.x, -1.5, 0.08);
        tuxGroup.position.y = THREE.MathUtils.lerp(tuxGroup.position.y, -0.6, 0.08); // Correction précédente
        ...
        tuxModel.rotation.z = 0; tuxModel.scale.y = 1;
        ...
        // Code existant pour le petit flottement (conservé)
        tuxGroup.position.y += Math.sin(Date.now() * 0.005) * 0.005;
    }
    else if (gameState === 'waiting') {
        tuxModel.scale.y = 1 + Math.sin(Date.now() * 0.003) * 0.02; // Respiration
        tuxModel.rotation.x = 0; tuxGroup.position.y = 0; headGroup.rotation.x = 0; feetGroup.rotation.x = 0;
...
function checkWin() {
    ...
    if (tuxGroup.position.z <= finishLineDist) {
        uiResultText.innerHTML = "<span class='win-text'>Champion !</span><br>Tux maîtrise la glisse !";
    } else {
        const remaining = Math.round(tuxGroup.position.z - finishLineDist);
        uiResultText.innerHTML = `<span class='lose-text'>Raté...</span><br>Encore ${remaining}m.<br>T'es pas le pinguin qui glisse le plus loin.`;
    }
}

// Fonction globale pour le bouton "Rejouer"
window.resetGame = function() {
    tuxGroup.position.set(0, 0, 0); tuxModel.rotation.set(0,0,0);
    gameState = 'waiting'; power = 0; velocity = 0;
    headGroup.rotation.x = 0; feetGroup.rotation.x = 0;
    flipperL.rotation.z = Math.PI/4; flipperR.rotation.z = -Math.PI/4;
    uiMessage.style.display = 'none'; uiStart.style.display = 'block'; uiBar.style.width = '0%';
}
